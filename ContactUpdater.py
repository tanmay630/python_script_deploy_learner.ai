...
class ContactUpdater:
    ...
    def insert_data(self):
        for i in range(2):
            first_name = f"First Name {random.randint(1,100)}"
    ...
    def update_contact(self, contact):
        url = f"https://api.hubapi.com/contacts/v1/contact/createOrUpdate/email/{contact[2]}"
        data = {
            "properties": [
                {
                    "property": "firstname",
        ...
        if response.status_code == 200:
            ...
            if "vid" in data:
                hubspot_id = data["vid"]
                self.cursor.execute(f"UPDATE [dbo].[contact] SET [hubspot_id] = {hubspot_id} WHERE [email] = '{contact[2]}'")
                self.conn.commit()
                print(f"Contact {contact[0]} {contact[1]} ({contact[2]}) updated successfully!")
            else:
                print(f"Failed to update contact {contact[0]} {contact[1]} ({contact[2]}).")
        else:
            print(f"Failed to update contact {contact[0]} {contact[1]} ({contact[2]}).")
...
if __name__ == "__main__":
    ...
    cu = ContactUpdater(client_id="d984097d-7fd1-4a8c-b39a-51c1bbcbe740", client_secret="eb596ae8-96ff-40b5-925c-4284b68dd1e9", refresh_token="eb596ae8-96ff-40b5-925c-4284b68dd1e9")
